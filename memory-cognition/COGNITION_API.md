...
这一包公开认知书的完整写入链路，而不只是四本书的 Markdown 文件：按日取证、世界事实核验、自我认识修订、他者认识分析、理论整理、版本冲突保护、维护记录、人工编辑与只读注入投影。宿主只需接入自己的权威对话、模型、数据根和触发时机。代码默认面向本地单用户固定主会话；HTTP 路由必须由宿主加鉴权，不能直接暴露在公网。
...
日结串行执行 world → self → other → scp。SCP 只有在真实的词条注入消息提交后调用 `cognition.scp_daily.mark(source_date, session_id, turn_id, entry_names)` 才会整理；没触发时返回 `not_triggered`。失败路保留已完成路的结果和维护记录，再次运行仍按来源摘要复用同一运行 ID；`run_daily` 会抛出 `BookError`，不要将部分成功显示成全部完成。运行状态与候选分别由 `cognition.maintenance.status()`、`record_report()` 查询。
...
| 用途 | 路径／方法 |
| --- | --- |
| 四书读取、手动写入与删除 | `GET /api/cognition/books/{domain}`；`PUT/DELETE /api/cognition/books/{domain}/{name}` |
...
| 日结执行、维护状态和理论触发状态 | `POST /api/cognition/maintenance/run`；`GET /api/cognition/maintenance`；`GET /api/cognition/maintenance/scp-status` |
...
只读注入可直接调用 `books.core_context()`、`books.person_projection(subject)`、`other_book.context(...)`。`self_book`／`world_book`／`scp_encyclopedia` 的检索类也可复用，但初始化时应显式把 `entries_dir` 指向 `books.folder(domain)`；这些旧检索类默认路径仍是源码目录，不会自动跟随 `books.ROOT`，写入后也要重新加载其缓存。四书文件、实体登记、阅读状态、维护账本与删除备份都写入 `books.ROOT`，请只放在私有数据目录并纳入自己的备份策略。
