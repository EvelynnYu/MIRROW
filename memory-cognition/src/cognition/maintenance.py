...
def validate(proposal, space):
    ...
    if p["domain"] == "world" and p["kind"] in {"self_reflection", "theory"}:
        raise books.BookError("自我或理论内容不能写入世界书")
    refs = p.get("evidence")
...
def make_plan(p, domain=None, name=None, operation=None):
    ...
    if domain == "self":
        ...
        base = {**base, "kind": "theory"}
    elif base["kind"] in {"theory", "self_reflection"}:
...
def auto_reason(p, review, plan, snapshot):
    ...
    if plan["domain"] == "world":
        ...
        # Established definitions are revised only after an explicit review.
        if plan["revision"] != "new" or review.get("agreed") is not True or {e["speaker"] for e in p["evidence"]} != {"agent", "human"}:
            return "SCP 定义修订或尚未得到双方明确一致"
    ...
    with books.LOCK:
        ...
        try:
            entry = books.save(plan["domain"], plan["name"], {**plan, "body": plan["after"]},
                               ...
                               editor=("Agent 自主整理自动更新" if plan['domain'] in {'self', 'other'} else "SCP 独立整理自动更新" if plan['domain'] == 'scp' else "世界书事实核验自动更新") if automatic else "人类伙伴确认认知维护候选",
